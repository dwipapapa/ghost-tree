namespace SpriteKind {
    export const Escapedplayer = SpriteKind.create()
}
controller.A.onEvent(ControllerButtonEvent.Pressed, function () {
    if ((mySprite.isHittingTile(CollisionDirection.Bottom) || mySprite.isHittingTile(CollisionDirection.Right) || mySprite.isHittingTile(CollisionDirection.Left)) && !(Stuck)) {
        mySprite.vy = -200
    }
})
controller.down.onEvent(ControllerButtonEvent.Released, function () {
    multilights.bandWidthOf(mySprite, 8)
    controller.moveSprite(mySprite, 100, 0)
    Stuck = false
})
controller.down.onEvent(ControllerButtonEvent.Pressed, function () {
    controller.moveSprite(mySprite, 0, 0)
    multilights.bandWidthOf(mySprite, 2)
    tiles.placeOnTile(mySprite, tiles.locationOfSprite(mySprite))
})
scene.onOverlapTile(SpriteKind.Player, assets.tile`myTile5`, function (sprite, location) {
    multilights.toggleLighting(false)
    tiles.setSmallTilemap(tilemap`level3`)
    sprite.setKind(SpriteKind.Escapedplayer)
...
multilights.addLightSource(mySprite, 8)
...
for (let value of tiles.getTilesByType(assets.tile`myTile2`)) {
    mySprite2 = sprites.create(img`
        ...
        . 1 . 1 . 1 . 1 
        `, SpriteKind.Enemy)
    multilights.addLightSource(mySprite2, 2)
...
game.onUpdateInterval(2000, function () {
    Random_Draw = randint(0, 50)
})
game.onUpdateInterval(500, function () {
    for (let index = 0; index < 2; index++) {
        nomiving = sprites.allOfKind(SpriteKind.Enemy)._pickRandom()
        for (let value of sprites.allOfKind(SpriteKind.Enemy)) {
            if (spriteutils.distanceBetween(mySprite, value) >= 64 && !(Stuck)) {
                spriteutils.setVelocityAtAngle(value, spriteutils.angleFrom(value, mySprite), 75)
            } else {
                nomiving.setVelocity(randint(-50, 50), randint(-50, 50))
            }
        }
        if (nomiving.vx < 0) {
            multilights.flashlightSourceAttachedTo(nomiving).direction = 180
        } else if (nomiving.vx > 0) {
            multilights.flashlightSourceAttachedTo(nomiving).direction = 0
        }
    }
})
